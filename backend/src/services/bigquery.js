  ...
  if (dbt) {
    ...
    query = `
      SELECT
        ...
        data_quality_score, port_time_hours, hours_in_port_so_far, last_updated_at
      FROM ${T.VESSELS}
      ${where}
  ...
  const [rows] = await bigquery.query({
    query: `
      SELECT
        ...
        ` : `
        0 AS underway, 0 AS in_port, 0 AS departed, 0 AS expected,
        0 AS avg_data_quality, 0 AS live_positions
        `}
      FROM ${table}
