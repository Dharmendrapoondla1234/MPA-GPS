// backend/src/routes/predict.js — AI Route Prediction v4
// Uses shared BigQuery client from services/bigquery.js
"use strict";
...
// Reuse the same authenticated BQ client that vessels route uses
...
function bearing(la1,lo1,la2,lo2){const r=Math.PI/180,dLo=(lo2-lo1)*r,y=Math.sin(dLo)*Math.cos(la2*r),x=Math.cos(la1*r)*Math.sin(la2*r)-Math.sin(la1*r)*Math.cos(la2*r)*Math.cos(dLo);return(Math.atan2(y,x)*180/Math.PI+360)%360;}
function formatETA(h){if(!h||h<0)return"Unknown";if(h<1)return`~${Math.round(h*60)} min`;if(h<24)return`~${h.toFixed(1)} hrs`;const d=Math.floor(h/24),rem=Math.round(h%24);return rem>0?`~${d}d ${rem}h`:`~${d} days`;}
function bqVal(v){if(v===null||v===undefined)return null;if(typeof v==="object"&&v.value!==undefined)return String(v.value).trim()||null;return String(v).trim()||null;}
...
router.get("/:imo", async (req, res, next) => {
  try {
    ...
    logger.info(`[PREDICT] querying vessel IMO ${imo}`);
    const [vesselRows] = await bigquery.query({
      query: `
        SELECT vessel_name, imo_number, flag, vessel_type,
    ...
    logger.info(`[PREDICT] vessel found: ${bqVal(vessel.vessel_name)}`);
    ...
    logger.info(`[PREDICT] history: ${hist.length} points`);

    const curLat = Number(vessel.latitude_degrees || 0);
    const curLng = Number(vessel.longitude_degrees || 0);

    if (!curLat || !curLng) {
      return res.status(422).json({ success:false, error:"No position data for vessel" });
    }

    // Avg heading/speed from last 10 AIS points
    ...
    const scored = PORTS.map(port => {
      ...
      const total  = hdgScore*1.8 + distScore + (isDecl ? 3.0 : 0);
    ...
    if (top) {
      ...
      for (let i=1; i<steps; i++) {
        const t=i/steps;
        waypoints.push({
          lat: curLat+(top.lat-curLat)*t, lng: curLng+(top.lng-curLng)*t,
    ...
    const result = {
      vessel: {
        name: bqVal(vessel.vessel_name)||"Unknown", imo,
        flag: bqVal(vessel.flag), lat: curLat, lng: curLng,
    ...
    setCache(ck, result);
    logger.info(`[PREDICT] IMO ${imo} → ${top?.port} (${top?.confidence}% conf)`);
    res.json({ success:true, cached:false, ...result });

  } catch(err) {
    logger.error(`[PREDICT] FAILED for IMO ${req.params.imo}: ${err.message}`);
