// backend/src/routes/predict.js — v10 (Photons_MPA dbt tables)
"use strict";
...
router.get("/ping", (_req,res) => res.json({ ok:true, router:"seaGraph-v10", dataset:"Photons_MPA" }));
...
router.get("/:imo", async (req,res) => {
  ...
  try {
    ...
    // Query from fct_vessel_master which has full enriched data
    const [vesselRows] = await bigquery.query({
      query:`
        SELECT vessel_name, latitude_degrees, longitude_degrees,
               ...
               vessel_status, berth_location, shipping_agent,
               crew_count, passenger_count, data_quality_score
        FROM ${T.MASTER}
    ...
    const scored = PORTS.map(port => {
      ...
      return {
        ...
        score:total, eta_hours:etaH?Math.round(etaH*10)/10:null,
        eta_iso:etaH?new Date(Date.now()+etaH*3600000).toISOString():null,
        eta_label:etaH?fmtETA(etaH):"Unknown", is_declared:!!isDecl,
    ...
    if(top) {
      ...
      wps=raw.map((pt,i)=>({
        ...
        eta_hours_from_now:top.eta_hours&&totalDist>0?Math.round(top.eta_hours*(cumDist[i]/totalDist)*10)/10:null,
      }));
    }
    ...
    const result = {
      vessel:{
        ...
        speed_kn:Math.round(avgSpd*10)/10, heading:Math.round(avgHdg),
        last_port:bqv(v.last_port_departed), declared_dest:declaredDest,
        vessel_status:bqv(v.vessel_status), shipping_agent:bqv(v.shipping_agent),
        crew_count:v.crew_count?Number(v.crew_count):null,
        data_quality:v.data_quality_score?Number(v.data_quality_score):null,
        speed_kmh: (Math.round(avgSpd*1852)/1000).toFixed(1),
      },
      prediction: top?{
        ...
        method:top.is_declared?"Declared destination confirmed":hist.length>20?"AIS trajectory + graph sea routing":"Heading & maritime graph analysis",
      ...
      analysis:{
        ...
        data_source:"Photons_MPA.fct_vessel_master",
      },
    };

    setCache(`pred_${imo}`, result);
    logger.info(`[PREDICT v10] IMO=${imo} → ${top?.port}`);
