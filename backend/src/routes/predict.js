// backend/src/routes/predict.js — AI Route Prediction v5
"use strict";

const express = require("express");
const router  = express.Router();
const logger  = require("../utils/logger");
const { bigquery, BQ_LOCATION, FULL_TABLE, SNAPSHOT_TABLE } = require("../services/bigquery");
...
function brng(la1,lo1,la2,lo2){const r=Math.PI/180,dLo=(lo2-lo1)*r,y=Math.sin(dLo)*Math.cos(la2*r),x=Math.cos(la1*r)*Math.sin(la2*r)-Math.sin(la1*r)*Math.cos(la2*r)*Math.cos(dLo);return(Math.atan2(y,x)*180/Math.PI+360)%360;}
...
router.get("/:imo", async (req, res) => {
  ...
  if (!bigquery) return res.status(500).json({ success:false, error:"BigQuery client not available" });
  ...
  try {
    ...
    let vesselRows;
    try {
      [vesselRows] = await bigquery.query({
        query:`SELECT vessel_name,latitude_degrees,longitude_degrees,speed,heading,course,flag,last_port_departed,next_port_destination FROM ${FULL_TABLE} WHERE imo_number=${imo} LIMIT 1`,
        location: BQ_LOCATION,
      });
    } catch(e) {
      logger.error(`[PREDICT] vessel query failed: ${e.message||JSON.stringify(e)}`);
      return res.status(500).json({ success:false, error:`BQ vessel query: ${e.message||"unknown"}` });
    }

    if (!vesselRows || vesselRows.length===0) return res.status(404).json({ success:false, error:"Vessel not found" });
    ...
    try {
      const [rows] = await bigquery.query({
        query:`SELECT latitude_degrees,longitude_degrees,speed,heading,course FROM ${SNAPSHOT_TABLE} WHERE imo_number=${imo} AND effective_timestamp>=TIMESTAMP_SUB(CURRENT_TIMESTAMP(),INTERVAL 72 HOUR) AND latitude_degrees IS NOT NULL ORDER BY effective_timestamp ASC LIMIT 500`,
        location: BQ_LOCATION,
      });
      hist = rows || [];
    } catch(e) {
      logger.warn(`[PREDICT] history query failed (non-fatal): ${e.message||JSON.stringify(e)}`);
    }
    logger.info(`[PREDICT] IMO=${imo} hist=${hist.length} lat=${curLat} lng=${curLng}`);
    ...
    const scored = PORTS.map(port=>{
      ...
      const b=brng(curLat,curLng,port.lat,port.lng);
      let diff=Math.abs(b-avgHdg); if(diff>180)diff=360-diff;
      const hdgS=Math.max(0,1-diff/90);
      const dstS=d<5?0:d<50?0.2:d<800?1.0:d<2000?0.5:0.1;
      const isD=declaredDest&&(declaredDest.toLowerCase().includes(port.name.toLowerCase().slice(0,6))||port.name.toLowerCase().includes(declaredDest.toLowerCase().slice(0,6))||declaredDest.toUpperCase().includes(port.code));
      const total=hdgS*1.8+dstS+(isD?3.0:0);
      const etaH=avgSpd>0.3?d/avgSpd:null;
      return{port:port.name,code:port.code,lat:port.lat,lng:port.lng,distance_nm:Math.round(d),bearing_deg:Math.round(b),heading_alignment:Math.round(hdgS*100),score:total,eta_hours:etaH?Math.round(etaH*10)/10:null,eta_iso:etaH?new Date(Date.now()+etaH*3600000).toISOString():null,eta_label:etaH?fmtETA(etaH):"Unknown",is_declared:!!isD,confidence:Math.min(Math.round(total*20),97)};
    }).filter(p=>p.distance_nm>3).sort((a,b)=>b.score-a.score);

    const top=scored[0], alts=scored.slice(1,4);
    ...
    if(top){
      ...
      for(let i=1;i<steps;i++){const t=i/steps;wps.push({lat:curLat+(top.lat-curLat)*t,lng:curLng+(top.lng-curLng)*t,label:`Waypoint ${i}`,type:"waypoint",eta_hours_from_now:top.eta_hours?Math.round(top.eta_hours*t*10)/10:null});}
      wps.push({lat:top.lat,lng:top.lng,label:top.port,type:"destination",eta_label:top.eta_label});
    }

    const result={
      vessel:{name:bqv(v.vessel_name)||"Unknown",imo,flag:bqv(v.flag),lat:curLat,lng:curLng,speed_kn:Math.round(avgSpd*10)/10,speed_kmh:Math.round(avgSpd*1.852*10)/10,heading:Math.round(avgHdg),last_port:bqv(v.last_port_departed),declared_dest:declaredDest},
      prediction:top?{destination:top.port,destination_code:top.code,destination_lat:top.lat,destination_lng:top.lng,eta_hours:top.eta_hours,eta_label:top.eta_label,eta_iso:top.eta_iso,distance_nm:top.distance_nm,bearing_deg:top.bearing_deg,confidence:top.confidence,is_declared:top.is_declared,method:top.is_declared?"Declared destination confirmed":hist.length>20?"AIS trajectory extrapolation":"Heading & proximity analysis"}:null,
      alternatives:alts,
      route_waypoints:wps,
      analysis:{history_points:hist.length,avg_speed_kn:Math.round(avgSpd*10)/10,avg_heading:Math.round(avgHdg),ports_scored:PORTS.length},
    };
    ...
    const code=err?.code||"";
    logger.error(`[PREDICT] UNHANDLED IMO=${imo} code=${code} msg=${msg}`);
