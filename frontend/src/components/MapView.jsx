...
const BASE_URL =
  process.env.REACT_APP_API_URL || "https://vessel-backends.onrender.com/api";
...
const MapView = forwardRef(function MapView(
  ...
  const [layers, setLayers] = useState({
    ...
    aiTrajectory: true, // OFF by default — too noisy in busy ports
  });
    ...
    loaderPromise.then(() => {
      ...
      const clusterRenderer = {
        render({ count, position }) {
          const col =
            count < 20 ? "#00e5ff" : count < 100 ? "#ffaa00" : "#ff3355";
  ...
  useEffect(() => {
    ...
    const NEAR = 0.015,
      cap = Math.min(moving.length, 80); // cap at 80 moving vessels
    for (let i = 0; i < cap; i++)
  ...
  useEffect(() => {
    ...
    fresh.forEach((v) => {
      ...
      if (markersRef.current[id]) {
        ...
        const posChanged =
          !prev ||
          Math.abs(Number(prev.latitude_degrees) - lat) > 0.00001 ||
          Math.abs(Number(prev.longitude_degrees) - lng) > 0.00001;
        const selChanged = prev?._isSel !== isSel;
        const spdChanged = prev?.speed !== v.speed;
        if (posChanged) smoothMove(m, lat, lng);
        if (posChanged || selChanged || spdChanged || al) {
          m.setIcon(getVesselIcon(v, isSel, al));
        ...
        const m = new window.google.maps.Marker({
          ...
          map: mapObj.current, // set map directly — belt-and-suspenders
        });
        m._vessel = v;
        m.addListener("click", () => {
          hoverWin.current.close();
        ...
        if (!IS_MOBILE) {
          m.addListener("mouseover", () => {
            const ve = m._vessel,
              spd = parseFloat(ve.speed || 0),
              col = getSpeedColor(spd),
              region = getRegionName(
                parseFloat(ve.latitude_degrees || 0),
                parseFloat(ve.longitude_degrees || 0),
              );
  ...
  useEffect(() => {
    ...
    const lat = Number(selectedVessel.latitude_degrees),
      lng = Number(selectedVessel.longitude_degrees);
    if (!lat || !lng) return;
    mapObj.current.panTo({ lat, lng });
    Object.values(markersRef.current).forEach((m) => {
      if (m._vessel)
        m.setIcon(
          getVesselIcon(
            m._vessel,
            m._vessel.imo_number === selectedVessel.imo_number,
            null,
          ),
        );
    });
  ...
  useEffect(() => {
    ...
    // Real trail
    const realPts = raw;
    const SEG = Math.min(realPts.length - 1, 50),
      step = Math.max(1, Math.floor((realPts.length - 1) / SEG));
    for (let i = 0; i < realPts.length - 1; i += step) {
      const end = Math.min(i + step + 1, realPts.length),
        prog = i / (realPts.length - 1);
      const r = Math.round(prog * 100),
        g = Math.round(150 + prog * 105),
        b = Math.round(200 + prog * 55);
  ...
  return (
    <div className="mv-root">
      ...
      <div className="mv-ctrl-cluster">
        ...
        <button
          className={`mv-ctrl-btn ${showLayerPanel ? "mv-ctrl-active" : ""}`}
          onClick={(e) => {
            e.stopPropagation();
        ...
        >
          <span>🗺️</span>
          <span className="mv-ctrl-txt">
            LAYERS{loadingGIS && <span className="mv-ctrl-dot" />}
          </span>
        </button>
        <div className="mv-zoom-group">
          <button
            ...
            onClick={() =>
              mapObj.current?.setZoom((mapObj.current.getZoom() || 10) + 1)
            }
          >
            +
          </button>
        ...
        {coords && !IS_MOBILE && (
          <div className="mv-bh-coords">
            ...
            {(() => {
              ...
              return r ? <span className="mv-bh-region"> · {r}</span> : null;
            })()}
          </div>
        )}
      </div>
...
function dangerInfoContent(f) {
  return `<div style="font-family:'JetBrains Mono',monospace;background:#1a0010;border:1px solid #ff2244;border-radius:8px;padding:10px 14px;color:#fff;min-width:200px"><div style="color:#ff2244;font-weight:700;font-size:12px">⛔ DANGER / HAZARD</div><div style="margin-top:6px;font-size:11px">${f.name || "Unknown Hazard"}</div>${f.depth !== undefined && f.depth !== null ? `<div style="font-size:10px;color:#ff8899;margin-top:3px">Depth: ${f.depth}m</div>` : ""}${f.info ? `<div style="margin-top:4px;font-size:9px;color:#cc8888">${String(f.info).substring(0, 200)}</div>` : ""}</div>`;
}
