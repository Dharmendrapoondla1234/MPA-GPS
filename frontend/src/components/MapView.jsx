// src/components/MapView.jsx — No full re-render, hover tooltip, trail fix
import React, { useEffect, useRef, useState, useCallback } from "react";
import { Loader } from "@googlemaps/js-api-loader";
import { MarkerClusterer } from "@googlemaps/markerclusterer";
import { getVesselIcon, buildInfoWindowContent, getSpeedColor } from "../utils/vesselUtils";
import "./MapView.css";
...
function smoothMove(marker, newLat, newLng, ms = 1600) {
  ...
  const t0 = performance.now(), lat0 = from.lat(), lng0 = from.lng();
  const step = (now) => {
...
export default function MapView({ vessels, selectedVessel, onVesselClick, trailData }) {
  ...
  useEffect(() => {
    ...
    loaderPromise.then(() => {
      ...
      hoverWin.current = new window.google.maps.InfoWindow({ maxWidth: 220, disableAutoPan: true });
          ...
          render({ count, position }) {
            return new window.google.maps.Marker({
              ...
              icon: {
                ...
                fillColor: "#00e5ff", fillOpacity: 0.14,
                strokeColor: "#00e5ff", strokeWeight: 1.5,
              },
              label: {
  ...
  useEffect(() => {
    ...
    vessels.forEach(v => {
      ...
      if (markersRef.current[id]) {
        // Update existing — smooth move, no flicker
        ...
        const m = new window.google.maps.Marker({
          position: { lat, lng }, icon: getVesselIcon(v, isSel),
          title: v.vessel_name || "Vessel", optimized: false,
        ...
        // Mouse over → quick tooltip
        m.addListener("mouseover", () => {
          const vessel = m._vessel;
          const spd = parseFloat(vessel.speed || 0);
          const col = getSpeedColor(spd);
          hoverWin.current.setContent(`
            <div style="font-family:'JetBrains Mono',monospace;background:#0b1525;border:1px solid ${col}55;
              border-radius:10px;padding:8px 12px;min-width:160px;color:#f0f8ff;box-shadow:0 8px 24px rgba(0,0,0,0.8)">
              <div style="font-size:12px;font-weight:700;color:#00e5ff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px;font-family:'Orbitron',monospace">
                ${vessel.vessel_name||"Unknown"}
              </div>
              <div style="margin-top:5px;display:flex;align-items:center;gap:8px">
                <span style="font-size:14px;font-weight:700;color:${col}">${spd.toFixed(1)} kn</span>
                <span style="font-size:9px;color:#3d6a8a">${vessel.heading||0}° HDG</span>
                <span style="font-size:9px;color:#3d6a8a">${vessel.flag||""}</span>
              </div>
              <div style="font-size:9px;color:#3d6a8a;margin-top:3px">
                ${(vessel.latitude_degrees||0).toFixed(4)}°N · ${(vessel.longitude_degrees||0).toFixed(4)}°E
              </div>
              <div style="font-size:8px;color:#3d6a8a;margin-top:2px;font-style:italic">Click for details</div>
            </div>
          `);
  ...
  useEffect(() => {
    ...
    pulseCirc.current = new window.google.maps.Circle({
      center:{lat,lng}, radius:700, fillColor:color, fillOpacity:0.06,
      strokeColor:color, strokeOpacity:0.7, strokeWeight:2, map:mapObj.current, zIndex:5,
    });
    let r=700, grow=true;
    pulseTimer.current = setInterval(() => {
      if (!pulseCirc.current) return;
      if (grow) { r+=55; if(r>1400) grow=false; } else { r-=55; if(r<700) grow=true; }
      try { pulseCirc.current.setRadius(r); pulseCirc.current.set("fillOpacity", 0.02+(1400-r)/30000); } catch(_){}
    }, 80);
  ...
  useEffect(() => {
    ...
    // Normalize BigQuery field name variants
    const pts = trailData.map(p => ({
    ...
    // Gradient polyline: dark (old) → bright cyan (recent)
    const SEG = Math.min(pts.length - 1, 30);
    const step = Math.max(1, Math.floor((pts.length-1) / SEG));
    for (let i = 0; i < pts.length-1; i += step) {
      const end  = Math.min(i+step+1, pts.length);
      const prog = i / (pts.length-1);
      const g = Math.round(80 + prog*149).toString(16).padStart(2,"0");
      const b = Math.round(120 + prog*135).toString(16).padStart(2,"0");
      trailObjs.current.push(new window.google.maps.Polyline({
        path: pts.slice(i, end), geodesic: true,
        strokeColor: `#00${g}${b}`,
        strokeOpacity: 0.25 + prog*0.75,
        strokeWeight: 2 + prog*3.5,
        map: mapObj.current, zIndex: 3,
      }));
    }

    // Direction arrows
    const asp = Math.max(1, Math.floor(pts.length / 8));
    for (let i = asp; i < pts.length; i += asp) {
      trailObjs.current.push(new window.google.maps.Polyline({
        path: [pts[i-1], pts[i]], strokeOpacity: 0,
        icons: [{ icon: { path: window.google.maps.SymbolPath.FORWARD_CLOSED_ARROW, scale:3.5, fillColor:"#00e5ff", fillOpacity:0.9, strokeColor:"#001a26", strokeWeight:1 }, offset:"100%" }],
        map: mapObj.current, zIndex: 4,
      }));
    }

    // Start dot
    trailObjs.current.push(new window.google.maps.Marker({
      position: pts[0],
      icon: { path: window.google.maps.SymbolPath.CIRCLE, scale:8, fillColor:"#ffffff", fillOpacity:1, strokeColor:"#00e5ff", strokeWeight:3 },
      title: "Journey Start", map: mapObj.current, zIndex: 8,
    }));

    // Current position dot (last point)
    trailObjs.current.push(new window.google.maps.Marker({
      position: pts[pts.length-1],
      icon: { path: window.google.maps.SymbolPath.CIRCLE, scale:10, fillColor:"#00e5ff", fillOpacity:1, strokeColor:"#ffffff", strokeWeight:2.5 },
      title: "Current Position", map: mapObj.current, zIndex: 9,
    }));

    // Journey start label
    const startLabel = new window.google.maps.InfoWindow({
      content: `<div style="font-family:'JetBrains Mono',monospace;font-size:10px;background:#00e5ff;color:#001018;font-weight:800;padding:4px 10px;border-radius:5px;white-space:nowrap">⚓ JOURNEY START</div>`,
      position: pts[0], pixelOffset: new window.google.maps.Size(0,-16),
    });
    startLabel.open(mapObj.current);
    trailObjs.current.push(startLabel);

    // Current position label
    const endLabel = new window.google.maps.InfoWindow({
      content: `<div style="font-family:'JetBrains Mono',monospace;font-size:10px;background:#ff3355;color:#fff;font-weight:800;padding:4px 10px;border-radius:5px;white-space:nowrap">📍 CURRENT</div>`,
      position: pts[pts.length-1], pixelOffset: new window.google.maps.Size(0,-16),
    });
    endLabel.open(mapObj.current);
    trailObjs.current.push(endLabel);
  ...
  const cycleStyle = useCallback(() => {
    if (!mapObj.current) return;
    const next = { nautical:"dark", dark:"satellite", satellite:"nautical" }[mapStyle];
    if (next === "satellite") { mapObj.current.setMapTypeId("hybrid"); mapObj.current.setOptions({ styles:[] }); }
    else { mapObj.current.setMapTypeId("roadmap"); mapObj.current.setOptions({ styles: next==="dark"?DARK_STYLE:NAUTICAL_STYLE }); }
    setMapStyle(next);
  }, [mapStyle]);
    ...
    <div className="mv-root">
      ...
      {trailData?.length > 0 && (
        <div className="mv-hud mv-trail-hud">
          📍 {trailData.length} pts · {selectedVessel?.vessel_name||""}
        </div>
      )}
      {coords && <div className="mv-coords">{coords.lat}° N &nbsp; {coords.lng}° E</div>}
    </div>
  );
}
...
const NAUTICAL_STYLE = [
  ...
  {featureType:"landscape",elementType:"geometry",stylers:[{color:"#2d5a27"}]},
  {featureType:"landscape.natural",elementType:"geometry",stylers:[{color:"#2a5524"}]},
  {featureType:"landscape.man_made",elementType:"geometry",stylers:[{color:"#384830"}]},
  {featureType:"road",elementType:"geometry",stylers:[{color:"#3a5030"}]},
  {featureType:"road",elementType:"geometry.stroke",stylers:[{color:"#2a3a20"}]},
  {featureType:"road",elementType:"labels.text.fill",stylers:[{color:"#6a9a60"}]},
  {featureType:"road",elementType:"labels.text.stroke",stylers:[{color:"#0a1a08"}]},
  {featureType:"administrative",elementType:"geometry.stroke",stylers:[{color:"#5a8a50"},{weight:1}]},
  {featureType:"administrative.country",elementType:"labels.text.fill",stylers:[{color:"#a0cca0"}]},
  {featureType:"administrative.locality",elementType:"labels.text.fill",stylers:[{color:"#88bb88"}]},
