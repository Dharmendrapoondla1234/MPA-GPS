...
export default function TopBar({ filters, onFiltersChange, vesselTypes, stats, nextRefresh, loading, onRefresh, panelOpen, onTogglePanel, user, onLogout }) {
    ...
    <header className="topbar">
      ...
      <div className="tb-logo">
        <div className="tb-sonar">
          <div className="tb-sonar-ring" /><div className="tb-sonar-ring" style={{animationDelay:"0.6s"}} />
      ...
      <div className="tb-filter-group">
        ...
        <select className="tb-select" value={filters.speedRange} onChange={e=>{
      ...
      <div className="tb-stats">
        ...
        <StatPill v={stats?Number(stats.moving_vessels||0).toLocaleString():"—"} l="MOVING"   c="green" />
        <div className="tb-divider" />
        <StatPill v={stats?`${parseFloat(stats.avg_speed||0).toFixed(1)}`:"—"} l="AVG KN"  c="amber" />
        <div className="tb-divider" />
        <StatPill v={stats?`${parseFloat(stats.max_speed||0).toFixed(1)}`:"—"} l="MAX KN"  c="red"   />
      </div>

      {/* Right controls */}
      <div className="tb-right">
        <div className={`tb-live ${loading?"loading":""}`}>
          <span className="tb-live-dot" />{loading?"SYNC":"LIVE"}
        </div>
        ...
        <button className={`tb-btn ${panelOpen?"active":""}`} onClick={onTogglePanel} title="Toggle panel">
        ...
        <div className="tb-user-wrap">
          <button className="tb-avatar" onClick={()=>setUserMenuOpen(o=>!o)} title={user?.email}>
            {user?.avatar || "?"}
          </button>
          {userMenuOpen && (
