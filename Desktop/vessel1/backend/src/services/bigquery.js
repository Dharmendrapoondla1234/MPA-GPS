...
// FIXED: Uses ADC locally, JSON credentials on Render
...
if (process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON) {
  ...
  try {
    ...
    logger.error("❌ GOOGLE_APPLICATION_CREDENTIALS_JSON is not valid JSON:", e.message);
    process.exit(1);
  }
  bigquery = new BigQuery({ credentials, projectId: PROJECT });
...
async function getLatestVessels({ search="", vesselType="", speedMin=null, speedMax=null, limit=5000 } = {}) {
  ...
  if (search) {
    ...
    conditions.push(`(LOWER(vessel_name) LIKE '%${s.toLowerCase()}%' OR CAST(imo_number AS STRING) LIKE '%${s}%')`);
  }
  ...
  logger.info(`[BQ] getLatestVessels → ${rows.length} rows in ${Date.now()-t0}ms`);
  return rows;
}
...
async function getVesselHistory(imoNumber) {
  ...
  const query = `
    SELECT vessel_name, imo_number, latitude_degrees, longitude_degrees,
           speed, heading, effective_timestamp
    FROM ${FULL_TABLE}
    WHERE imo_number = ${parseInt(imoNumber)}
    ORDER BY effective_timestamp DESC LIMIT 500
  `;
  const [rows] = await bigquery.query({ query });
  return rows;
}
...
async function getVesselTypes() {
  const [rows] = await bigquery.query({
    query: `SELECT DISTINCT vessel_type FROM ${FULL_TABLE} WHERE vessel_type IS NOT NULL ORDER BY vessel_type LIMIT 100`
  });
...
async function getFleetStats() {
  const [rows] = await bigquery.query({
    query: `SELECT COUNT(*) AS total_vessels, COUNTIF(speed > 0.5) AS moving_vessels,
      COUNTIF(speed <= 0.5 OR speed IS NULL) AS stationary_vessels,
      ROUND(AVG(speed),2) AS avg_speed, ROUND(MAX(speed),2) AS max_speed,
      COUNT(DISTINCT vessel_type) AS vessel_type_count FROM ${FULL_TABLE}`
  });
  return rows[0] || {};
}
...
module.exports = { getLatestVessels, getVesselHistory, getVesselTypes, getFleetStats, healthCheck };
